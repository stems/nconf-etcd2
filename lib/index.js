...
function getNamespaceNode(nodes, namespace, separator) {
	var node = nodes;
	var namespaceParts = namespace.split(separator || defaults.logicalSeparator);
	var namespacePartsLength = namespaceParts && namespaceParts.length || 0;
	for (var i = 0; i < namespacePartsLength; i++) {
		node = node[namespaceParts[i]] || node;
	}
	return node;
}
...
function Store( options ){
	...
	this.client           = options.client || new Etcd(options.hosts, options.options);
}
...
Store.prototype.load = function load( cb ){
	this.client.mkdir('/' + this.namespace.replace(this.logicalSeparator, '/'),function(){
		this.client.get('/' + this.namespace.replace(this.logicalSeparator, '/'), {recursive:true}, function(err, data ){
			(cb||NOOP)(err, err ? null : this.store = getNamespaceNode(rebuild(data.node), this.namespace, this.logicalSeparator));
		}.bind( this ));
	}.bind(this))
};
...
Store.prototype.loadSync = function load( cb ){
	var response = this.client.getSync('/' + this.namespace.replace(this.logicalSeparator, '/'), {recursive:true});

	if(response.err){
		if( response.err.errorCode == 100){
			this.client.mkdirSync('/' + this.namespace.replace(this.logicalSeparator, '/'))
			return this.store = {};
		} else {
			throw response.err.error;
		}
	} 
	return this.store = getNamespaceNode(rebuild(response.body.node), this.namespace, this.logicalSeparator);
};
