...
function getNamespaceNode(nodes, namespace) {
	var node = nodes;
...
Store.prototype.load = function load( cb ){
	this.client.mkdir('/' + this.namespace,function(){
		this.client.get('/' + this.namespace, {recursive:true}, function(err, data ){
			(cb||NOOP)(err, err ? null : this.store = getNamespaceNode(rebuild(data.node), this.namespace));
		}.bind( this ));
	}.bind(this))
};
...
Store.prototype.loadSync = function load( cb ){
	...
	return this.store = getNamespaceNode(rebuild(response.body.node), this.namespace);
};
